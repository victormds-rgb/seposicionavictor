...
import { OpenRouterPortaDeIA } from "@/ia/infrastructure/providers/openrouter-provider";
 ...
 * `OpenRouterPortaDeIA`, que lança erro se `OPENROUTER_API_KEY` não
 * estiver configurada. Só fora de produção, e só quando a chave está
 * ausente, cai no adapter fake — nunca silenciosamente em produção.
 */
function criarPortaDeIA(): PortaDeIA {
  if (!env.OPENROUTER_API_KEY && env.NODE_ENV !== "production") {
    return new FakeLocalPortaDeIA();
  }
  return new OpenRouterPortaDeIA();
}
